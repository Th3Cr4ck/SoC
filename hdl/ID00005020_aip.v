// `define ID00005020_MEM_IN 0
`define ID00005020_MEM_OUT 1

`define ID00005020_CONF_REG 1


// `define ID00005020_STREAMING_IN
// `define ID00005020_STREAMING_OUT

module ID00005020_aip
(
  clk,
  rst,
  en,
  ...
  output startIPcore;
  output dataConfigRegWritten;

  ID00005020_aipModules AIP
  (
    .clk (clk),
    .rst (rst),
    .en (en),
    ...
    .dataConfigRegWritten(dataConfigRegWritten)
  );

endmodule

module ID00005020_aipModules
(
  clk,
  rst,
  en,
  ...
  //--- IP-core ---//
  `ifdef ID00005020_MEM_IN
  rdDataMemIn,
  rdAddrMemIn,
  `elsif ID00005020_STREAMING_IN
  rdDataMemIn,
  rdAddrMemIn,
  `endif

  `ifdef ID00005020_MEM_OUT
  wrDataMemOut,
  wrAddrMemOut,
  wrEnMemOut,
  `elsif ID00005020_STREAMING_OUT
  wrDataMemOut,
  wrEnMemOut,
  `endif

  `ifdef ID00005020_CONF_REG
  rdDataConfigReg,
  `endif

  `ifdef ID00005020_STREAMING_IN
  dataInStreaming,
  startStreaming,
  wrInStreaming,
  `endif

  `ifdef ID00005020_STREAMING_OUT
  dataOutStreaming,
  doneStreaming,
  wrOutStreaming,
  `endif
  ...
  intIPCore,
  startIPcore,
  dataConfigRegWritten // Usr Defined
);

  localparam IP_ID = 32'h00005020;
  ...
  `ifdef ID00005020_MEM_IN
  localparam [(`ID00005020_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] MEM_IN_ADDR_WIDTH = {  };

  localparam [((`ID00005020_MEM_IN*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_IN = {  };
  `endif

  `ifdef ID00005020_MEM_OUT
  localparam [(`ID00005020_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] MEM_OUT_ADDR_WIDTH = { 16'd1 };

  localparam [((`ID00005020_MEM_OUT*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_OUT = { 5'b00001, 5'b00000 };
  `endif

  `ifdef ID00005020_CONF_REG
  localparam [(`ID00005020_CONF_REG*CONF_REG_ADDR_MAX_WIDTH)-1:0] CONF_REG_ADDR_WIDTH = { 3'd2 };
  ...
  `ifdef ID00005020_MEM_IN
  output [(`ID00005020_MEM_IN*DATA_WIDTH)-1:0] rdDataMemIn;
  input wire [(`ID00005020_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemIn;

  wire [(`ID00005020_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemIn;
  wire [`ID00005020_MEM_IN-1:0] wrEnMemIn;

  wire [DATA_WIDTH-1:0] rdDataMemIn0;
  `elsif ID00005020_STREAMING_IN
  output [DATA_WIDTH-1:0] rdDataMemIn;
  input wire [MEM_ADDR_MAX_WIDTH-1:0] rdAddrMemIn;
  `endif // ID00005020_MEM_IN

  `ifdef ID00005020_MEM_OUT
  input [(`ID00005020_MEM_OUT*DATA_WIDTH)-1:0] wrDataMemOut;
  input [(`ID00005020_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemOut;
  input [`ID00005020_MEM_OUT-1:0] wrEnMemOut;

  wire [(`ID00005020_MEM_OUT*DATA_WIDTH)-1:0] rdDataMemOut;
  wire [(`ID00005020_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemOut;
  `elsif ID00005020_STREAMING_OUT
  input [DATA_WIDTH-1:0] wrDataMemOut;
  input [0:0] wrEnMemOut;
  `endif // ID00005020_MEM_OUT
  ...
  output startIPcore;
  output reg dataConfigRegWritten; // Usr defined
  ...
  `ifdef ID00005020_MEM_IN
  generate
      for(i=0; i<`ID00005020_MEM_IN; i = i + 1) begin :  MEMIN
          if (0 == i)
  ...
  `ifdef ID00005020_MEM_OUT
  generate
      for(i=0; i<`ID00005020_MEM_OUT; i = i+1) begin : MEMOUT
          simple_dual_port_ram_single_clk
          #(
              .DATA_WIDTH (DATA_WIDTH),
  ...
  generate
      ...
      // DATA_REG interface logic
      always @(posedge clk or negedge rst)
        if (!rst)
          dataConfigRegWritten <= 1'b0;
        else if (wrEnConfigReg && (wrAddrConfigReg == CONF_REG_ADDR_WIDTH-1)) // If last reg written
          dataConfigRegWritten <= 1'b1;
        else 
          dataConfigRegWritten <= 1'b0;
...
(
  ...
  `ifdef ID00005020_MEM_IN
  parameter [((`ID00005020_MEM_IN*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_IN = {5'b00011, 5'b00010, 5'b00001, 5'b00000};
  `endif

  `ifdef ID00005020_MEM_OUT
  parameter [((`ID00005020_MEM_OUT*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_OUT = {5'b00101, 5'b00100};
  `endif
  ...
  `ifdef ID00005020_MEM_IN
  output wire [(`ID00005020_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemIn;
  output wire [`ID00005020_MEM_IN-1:0] wrEnMemIn;

  reg [MEM_ADDR_MAX_WIDTH-1:0] regWrAddrMemIn [0:`ID00005020_MEM_IN];
  `endif

  `ifdef ID00005020_MEM_OUT
  output wire [(`ID00005020_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemOut;

  reg [MEM_ADDR_MAX_WIDTH-1:0] regRdAddrMemOut [0:`ID00005020_MEM_OUT];
  `endif
  ...
  `ifdef ID00005020_MEM_IN
  generate
      for (indexEn=0; indexEn<`ID00005020_MEM_IN; indexEn= indexEn + 1) begin : WR_EN_MEMIN
          assign wrEnMemIn[indexEn] = writeAIP & (configAIP == CONFIG_MEM_IN[CONFIG_WIDTH*(indexEn*2)+:CONFIG_WIDTH]);
      end // for ID00005020_MEM_IN
   endgenerate
  `endif
  ...
  always @(posedge clk or negedge rst) begin
    if (!rst) begin
      `ifdef ID00005020_MEM_IN
      for (i=0; i<`ID00005020_MEM_IN; i= i + 1) begin
        regWrAddrMemIn[i] <= 'd0;
      end
      `endif

      `ifdef ID00005020_MEM_OUT
      for (i=0; i<`ID00005020_MEM_OUT; i= i + 1) begin
  ...
  `ifdef ID00005020_MEM_IN
  generate
    for (indexAddr=0; indexAddr<(`ID00005020_MEM_IN); indexAddr=indexAddr+1) begin: MEM_IN_ADDR
  ...
  `ifdef ID00005020_MEM_OUT
  generate
    for (indexAddr=0; indexAddr<(`ID00005020_MEM_OUT); indexAddr=indexAddr+1) begin: MEM_OUT_ADDR
